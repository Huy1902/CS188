...
def sentence1() -> Expr:
    ...
    A or B statement 1
    (not A) if and only if ((not B) or C) statement 2
    (not A) or (not B) or C statement 3
    """
    ...
    not_B = ~B
    C = Expr('C')
    statement2 = (not_A) % (not_B | C)
    statement3 = disjoin([not_A, not_B, C])
...
def sentence2() -> Expr:
    ...
    C if and only if (B or D) statement 1
    A implies ((not B) and (not D)) statement 2
    (not (B and (not C))) implies A statement 3
    (not D) implies C statement 4
    """
    "*** BEGIN YOUR CODE HERE ***"
    A = Expr('A')
    B = Expr('B')
...
def sentence3() -> Expr:
    ...
    not killed at time 0 or it was not alive at time 0 and it was born at time 0. statement 1

    Pacman cannot both be alive at time 0 and be born at time 0. statement 2

    Pacman is born at time 0. statement 3
    """
    "*** BEGIN YOUR CODE HERE ***"
...
def findModel(sentence: Expr) -> Dict[Expr, bool]:
    ...
    """
    # Conjunctive normal form (the form required by SAT solver)
...
def findModelUnderstandingCheck() -> Dict[Expr, bool]:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    # a.__dict__['op'] = 'a'
    # print("a.__dict__ is:", a) # might be helpful for getting ideas
    # # a['op'] : 'a'
    # # a['args'] : ()
    a = tempClass('a')
    return {a: True}
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None,
    ...
    "*** BEGIN YOUR CODE HERE ***"
    pacman_not_at_location = conjoin([PropSymbolExpr(wall_str, x, y)
                                      >> ~PropSymbolExpr(pacman_str, x, y, time=t)
                                      for x, y in all_coords])
    pacman_at_location = exactlyOne([PropSymbolExpr(pacman_str, x, y, time=t)
                                     for x, y in non_outer_wall_coords])

    direction_sentence = exactlyOne([PropSymbolExpr(direction, time=t) for direction in DIRECTIONS])
    pacphysics_sentences += [pacman_not_at_location, pacman_at_location, direction_sentence]

    if sensorModel:
        sensor_model_sentence = sensorModel(t, non_outer_wall_coords)
        pacphysics_sentences.append(sensor_model_sentence)

    if successorAxioms and walls_grid and t:
        successor_model_sentence = successorAxioms(t, walls_grid, non_outer_wall_coords)
        pacphysics_sentences.append(successor_model_sentence)
