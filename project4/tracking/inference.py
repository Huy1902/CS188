...
def constructBayesNet(gameState: hunters.GameState):
    ...
    variables = []
    edges = []
    variableDomainsDict = {GHOST0: [], GHOST1: [], PAC: [], OBS0: [], OBS1: []}
    ...
    for x in range(X_RANGE):
        for y in range(Y_RANGE):
            variableDomainsDict[GHOST0] += [(x, y)]
            variableDomainsDict[GHOST1] += [(x, y)]
            variableDomainsDict[PAC] += [(x, y)]
    for pacPos in variableDomainsDict[PAC]:
        for ghostPos in variableDomainsDict[GHOST0]:
            distance = manhattanDistance(pacPos, ghostPos)
            if distance not in variableDomainsDict[OBS0]:
        ...
        for ghostPos in variableDomainsDict[GHOST1]:
            distance = manhattanDistance(pacPos, ghostPos)
            if distance not in variableDomainsDict[OBS1]:
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict,
        ...
        # grab all factors in bayes net where we know the evidence variables(aready assign value)
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        ...
        for eliminatedVariable in eliminationOrder:
            ...
            # print(f"eliminatedVariable{eliminatedVariable}")
            if len(joinedFactor.unconditionedVariables()) > 1:
                joinedFactor = eliminate(joinedFactor, eliminatedVariable)
                # print(f"joinedFactor{joinedFactor}")
                currentFactorsList.append(joinedFactor)

        fullJoin = joinFactors(currentFactorsList)

        queryConditionedOnEvidence = normalize(fullJoin)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        totalProbability = 0
        for key in self.keys():
            if totalProbability <= random_number < totalProbability + self[key]:
                return key
            totalProbability += self[key]
        "*** END YOUR CODE HERE ***"
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: int, pacmanPosition: Tuple, ghostPosition: Tuple, jailPosition: Tuple):
        ...
        """
        "*** YOUR CODE HERE ***"
        # noisy distance is observation
        if noisyDistance is None:
            # if ghost in jail then observation is 1 with probability is 1
            if ghostPosition == jailPosition:
                return 1
            # anything else with probability zero
            return 0
        # if ghost in jail then except observation is none, anything else with probability zero
        if ghostPosition == jailPosition:
            return 0
        # print(f"noisyDistance: {noisyDistance}")
        # print(f"pacmanPosition {pacmanPosition}")
        # print(f"ghostPosition {ghostPosition}")
        # print(f"jailPosition {jailPosition}")
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        for oldPos in self.allPositions:
            dictNewPosDis[oldPos] = self.getPositionDistribution(gameState, oldPos)

        newBeliefs = DiscreteDistribution()
        for oldPos in self.allPositions:
            for newPos in self.allPositions:
                newBeliefs[newPos] += dictNewPosDis[oldPos][newPos] * self.beliefs[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        "*** END YOUR CODE HERE ***"
        num_append = self.numParticles // len(self.legalPositions)
        for pos in self.legalPositions:
            for _ in range(num_append):
                self.particles.append(pos)
        for _ in range(self.numParticles % len(self.legalPositions)):
            self.particles.append(self.legalPositions[_])
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for position in self.allPositions:
            beliefs[position] = self.getObservationProb(observation, gameState.getPacmanPosition(),
                                                        position, self.getJailPosition()) * beliefs[position]
        if beliefs.total() == 0:
            self.initializeUniformly(gameState)
        else:
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.allPositions:
            for newPos in self.allPositions:
                newBeliefs[newPos] += dictNewPosDis[oldPos][newPos] * beliefs[oldPos]
        beliefs = newBeliefs
        beliefs.normalize()
