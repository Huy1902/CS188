...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        "*** YOUR CODE HERE ***"
        # for action in legal:
        #     print(Actions.getSuccessor(pacmanPosition, action))
        mostLikelyPos = []
        for beliefs in livingGhostPositionDistributions:
            maxPossibility = max([beliefs[pos] for pos in beliefs.keys()])
            for pos in beliefs.keys():
                if beliefs[pos] == maxPossibility:
                    mostLikelyPos.append(pos)
                    break

        # print(mostLikelyPos)
        distance = [self.distancer.getDistance(pacmanPosition, posLikelyGhost) for posLikelyGhost in mostLikelyPos]
        minDistance = min(distance)
        nearestGhost = None
        for i in range(len(mostLikelyPos)):
            if distance[i] == minDistance:
                nearestGhost = mostLikelyPos[i]
                break

        minDistance = MAXINT
        chosenAction = None
        for action in legal:
            distance = self.distancer.getDistance(nearestGhost, Actions.getSuccessor(pacmanPosition, action))
